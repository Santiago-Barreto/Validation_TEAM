...
export function useTeamLayers({
  ...
  useEffect(() => {
    ...
    const style = encodeURIComponent(landsatStyle || "green");

    fetch(`${API_URL}/landsat?year=${landsatYear}&style=${style}&${bq}`)
      .then(async (r) => {
        if (!r.ok) {
  ...
  }, [year, layersVisible.landsat, executed, biomas, landsatStyle]);
